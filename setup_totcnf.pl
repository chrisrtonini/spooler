...
# ----------------------------------------------------------------------------
# Utilitário para carregar tabela de Totalizadores Nao Sujeitos ao ICMS,
# a partir de um arquivo de texto contendo um registro por linha, de acordo
# com o seguinte formato:
# DESCR. TOTALIZADOR:T
# Onde "T" pode ser "S" indicando totalizador de SAIDA ou "E" para 
...
sub set_totalizador	{
	...
	print "Programar: [$descr][$tipo]\n";
	
	open($hnd, ">", $arq_tmp) 
		or confess "set_totalizador: erro ao criar arquivo $arq_tmp ($!)\n";
	
	print $hnd "<ecf_sgi_begin>\n";
	print $hnd "ecf_execute_request\n";
	print $hnd "session_id:$id_sessao\n";
	print $hnd "command_id:set_totalizador\n";
	print $hnd "valor:$descr\n";
	print $hnd "tipo:$tipo\n";
	print $hnd "<ecf_sgi_end>\n";
	...
	confess "set_totalizador: timeout (30s) !\n" if (!wait_for_lock());
	
	open($hnd, "<", $arq_ret)
		or confess "set_totalizador: erro ao abrir arquivo $arq_ret ($!)\n";
...
sub usage	{
	print STDERR "\n-= Utilitario para setup de totalizadores nao fiscais =-\n";
	print STDERR "\nSintaxe:\n\tperl setup_totcnf.pl -f <arquivo.txt>\n";
	print STDERR "Onde:\n\t<arquivo.txt> contem lista de totalizadores nao ";
	print STDERR "fiscais.\n\n";
	print STDERR "Observacao:\n";
	print STDERR "\tCada linha do arquivo estara no seguinte formato:\n";
	print STDERR "\t\t\"DESCR. TOT.:X\"\n";
	print STDERR "\tConsiderando que \"X\" tera valor \"E\" para ENTRADA, e ";
	print STDERR "\"S\" para SAIDA,\n";
	print STDERR "\te que a descricao sera limitada pelo tamanho maximo ";
	print STDERR "permitido do ECF.\n\n";
}
...
if ($cmdline)	{
	if (length($tot_file) == 0)	{
		...
		my $list;
		open($list, "<", $tot_file)
			or confess "Erro ao abrir arquivo $tot_file ($!)\n";
		
		while (my $item = <$list>)	{
			chomp($item);
			my ($nome_tot, $tipo_tot) = split(/:/, $item);
			
			set_totalizador($nome_tot, $tipo_tot);
		}
		close($list);
		...
		confess "Arquivo $tot_file nao pode ser lido!\n";
	}
}
